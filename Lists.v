...
Fixpoint oddmembers (l:natlist) : natlist :=
  match l with
    | [] => []
    | x :: xs => if even x then oddmembers xs else x :: (oddmembers xs)
  end.
...
Fixpoint included (s1 : bag) (s2 : bag) : bool :=
  match s1 with
    | [] => true
    | x :: xs => (negb (count x s2 =? 0)) && (included xs (remove_one x s2))
  end. 
...
Proof.
  intros v s. simpl. rewrite eqb_refl. rewrite add_comm. reflexivity.
Qed.
...
Proof.
  intros l1 l2.
  induction l1 as [|x1 l1' Hl1].
  - simpl. rewrite app_nil_r. reflexivity.
  - simpl. rewrite Hl1. Search ( (?l ++ ?m) ++ ?r = ?l ++ ?m ++ ?r ).
...
Theorem rev_involutive : forall l : natlist,
  rev (rev l) = l.
Proof.
  induction l as [|x l' HR].
  - reflexivity.
  - simpl. rewrite rev_app_distr. simpl. rewrite HR. reflexivity.
Qed. 
...
Proof.
  intros l1 l2.
  induction l1 as [|x1 l1' HRl1].
  - reflexivity.
  - destruct x1 as [|x1'].
    + simpl. rewrite HRl1. reflexivity.
    + simpl. rewrite HRl1. reflexivity.
Qed.
(** [] *)
...
Lemma le_succ : forall (n : nat), n <=? S n = true.
Proof.
  induction n as [|n' HR].
  - reflexivity.
...
Proof.
  induction s as [| x s' HR].
  - reflexivity.
  - simpl. destruct x.
    + simpl. rewrite le_succ. reflexivity.
    + simpl. rewrite HR. reflexivity.
Qed.
(** [] *)
...
Proof.
  intros v s1 s2.
  induction s1 as [|x s1' HR].
  - reflexivity.
  - simpl. rewrite HR. rewrite add_assoc. reflexivity. 
Qed.
...
Proof.
  intros f inv x y H.
  rewrite -> inv. rewrite <- H. rewrite <- inv. reflexivity.
Qed.
