...
Theorem map_app_distr : forall (X Y : Type) (f : X -> Y) (l1 : list X) (l2 :  list X),
...
Theorem map_rev_comm : forall (X Y : Type) (f : X -> Y) (l : list X),
...
Proof.
  ...
  induction l as [|x l].
  - reflexivity.
  - simpl. rewrite map_app_distr. rewrite map_rev_comm. reflexivity.
Qed. 
(** [] *)
...
(* Par exemple, X = list nat, Y = nat et f : l x -> length l + x.
    fold f l 0 permet alors de renvoyer le nombre d'élements au total contenus dans les sous-listes de l. *)
...
*)
(* 
  We proceed by induction. Let P(n) := "If length l = n, then @nth_error X l n = None."
  Initialisation : if n = 0, then l = [] and the conclusion follows by definition.
  Induction step : Assume P(n). We have to prove P(n + 1), ie assuming length l = n + 1, then @nth_error X l (n + 1) = None.
  Since length l = n + 1, there exists a list (x : X), (l' : list X) such that length l' = n and l = x :: l'. By rewriting the definition of nth_error, @nth_error X l (n + 1) = @nth_error X l' n = None, thanks to P(n).
 *)
...
Definition exp (n m : cnat) : cnat :=
  fun X => m ((X -> X) -> X -> X) (fun acc f x => n X (acc f) x) (one X).
