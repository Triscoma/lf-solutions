...
(* [apply] is useful to start from the end of the proof, when we think that the final result is a consequence of some lemma / hypothesis that we will have to prove after. [rewrite] serves to progress from the hypothesis, and it seems to be more useful to work with equalities, when [apply] is used with implications.
Both can usefully be applied when the result is among the hypothesis.
...
(* Let's show it by induction on n. Let P(n) := "forall m, if n ?= m then n = m."

  Initialisation :
  
    let m be a natural. 
    
    Thus m is equal to 0, or to S m', for a certain natural m'.
    
    In the first case, P(0) becomes if 0 ?= 0 then 0 = 0, 
    which is always true. 
    
    In the second case, P(0) is : if 0 ?= S m' then 0 = S m'. 
    By definition of =?, the hypothesis leads to an absurdity, and concludes the initialisation.
    
  Induction step : 
    
    Assume P(n), we aim at showing P(n+1).
    
    Let m be a natural number, as in the init two cases are possible.
    
    In the first one, since n + 1 != 0, we have an absurdity by definition of ?=.
    
    In the second one, when m = S m', P(n+1) rewrites n =? m' implies S n = S m', which is true after applying the induction hypothesis on n =? m' and using the fact that n = m implies S n = S m. 
     *)
...
Proof.
  intros X Y l.
  induction l as [| (x, y) l' HR].
  {
    intros l1 l2 H. simpl in H. injection H as H.
    rewrite <- H. rewrite <- H0. reflexivity.
  }
  {
    intros l1 l2 H. simpl in H. destruct (split l'). injection H as H.
    rewrite <- H. rewrite <- H0. simpl.
    assert (combine x0 y0 = l').
    {
...
(** **** Exercise: 3 stars, advanced, optional (eqb_sym_informal)
   ...
   (* 
      By induction on n. 
      Let P(n) := "for all natural m, (n =? m) = (m =? n)".
      
      Initialisation : We have to prove for all m, (0 =? m) = (m =? 0).
        if m = 0 it is obviously true, and else we can write m = S m' and the result follows immediately.
        
      Induction step : Assume P(n).
        We have to show that for all m, (n+1 =? m) = (m =? n+1).
        Again 2 cases:
          - if m = 0 then it is trivial.
          - else m = S m' for a certain natural m', and by definition it reduces to (n =? m') = (m' =? n), which is true by P(n).
    [] *)
...
Theorem len_0_empty : forall (X : Type) (l : list X),
  length l = 0 -> l = [].
Proof.
  destruct l.
  - reflexivity.
...
Proof.
  ...
  {
    intros l2 H. symmetry in H. simpl in H. apply len_0_empty in H.
...
Proof.
  ...
  {
    intros x lf H. simpl in H. destruct (test x0) eqn : Tx0.
    - injection H as H0 H1. rewrite <- H0. apply Tx0.
...
Example test_forallb_4 : forallb (fun x => 5 =? x) [5] = true.
Proof. reflexivity. Qed.

Fixpoint existsb {X : Type} (test : X -> bool) (l : list X) : bool :=
  match l with
    | [] => false
    | x :: xf => if test x then true else existsb test xf
  end.

Example test_existsb_1 : existsb (fun x => 5 =? x) [0;2;3;6] = false.
 reflexivity. Qed.
...
Proof. 
  ...
  - simpl. destruct (test x) eqn : Tx.
    {
      unfold existsb'. simpl. rewrite Tx. simpl. reflexivity.
    }
    {
      rewrite IHl'. unfold existsb'. simpl. rewrite Tx. reflexivity.
    }
Qed.
